...
- Request a new ACM certificate for each REST API of each deploy stage via AMC console (to be used by custom domains) and create the Route53 record from the ACM console during validation pending
- Create custom domain of each API on each stage and select the existing ACM certificates
